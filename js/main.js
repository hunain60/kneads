...
function onScroll(event){
		var scrollPos = sjq(document).scrollTop();
		sjq('#nav a').each(function () {
			var currLink = sjq(this);
			var refElement = sjq(currLink.attr("href"));
			if (refElement.position().top <= scrollPos && refElement.position().top + refElement.height() > scrollPos) {
				sjq('#nav ul li a').removeClass("active");
...
sjq(document).ready(function($) {
	...
	$('#full-video').videoBG({
		mp4:'media/cus.m4v',
		//poster:'images/PlainCushion.png',
		ogv:'media/cus.ogv',
		webm:'media/cus.webm',
		scale:true,
		muted:true,
    ...
    function miracle_sticky_header() {
        if ($("body").hasClass("no-sticky-menu")) {
			...
			$("#header_container").addClass("box-shadow-bottom");
            return;
        }
        ...
        if ($(window).scrollTop() > sticky_header_offset_top) {
			...
			$("#header_container").removeClass("box-shadow-bottom");
        } else {
            $("#header").removeClass("header-sticky");
			$("#header_container").addClass("box-shadow-bottom");
        }
	...
	function miracleResizeSoapMapHeight() {
		if ($(".page-title-container .google-map").length > 0) {
			var ratio = stGlobals.isMobile ? 0.8 : 1;
			$(".page-title-container .google-map").css("max-height", $(window).height() * ratio);
		}
	}
